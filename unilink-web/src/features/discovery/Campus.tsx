      ...
      <Container>
        ...
        <Content>
          ...
          <SectionHeader>
            ...
            <SeeAll onClick={() => setExpandirCursos(!expandirCursos)}>
              {expandirCursos ? "Mostrar menos" : "Ver todos"}
            </SeeAll>
          </SectionHeader>
          <ResponsiveList expandido={expandirCursos}>
            {listaCursos.map((curso, index) => (
              <Card key={curso.id}>
                ...
                <CardButtonDark onClick={() => alert(`Navegando para o curso: ${curso.nome}`)}>
                  VER CURSO
                </CardButtonDark>
              </Card>
            ))}
          ...
          <SectionHeader>
            ...
            <SeeAll onClick={() => setExpandirProjetos(!expandirProjetos)}>
              {expandirProjetos ? "Mostrar menos" : "Ver todos"}
            </SeeAll>
          </SectionHeader>
          <ResponsiveList expandido={expandirProjetos}>
            {listaProjetos.map((proj, index) => (
              <Card key={index}>
                ...
                <CardButtonLight onClick={() => alert(`Conhecendo o projeto: ${proj.nome}`)}>
                  CONHECER
                </CardButtonLight>
              </Card>
            ))}
          ...
          <SectionHeader>
            ...
            <SeeAll onClick={() => setExpandirEventos(!expandirEventos)}>
              {expandirEventos ? "Mostrar menos" : "Ver todos"}
            </SeeAll>
          </SectionHeader>
          <ResponsiveList expandido={expandirEventos}>
            {listaEventos.map((evento, index) => (
              <Card key={index}>
                ...
                <CardButtonDark onClick={() => alert(`Inscrição no evento: ${evento.nome}`)}>
                  PARTICIPAR
                </CardButtonDark>
              </Card>
            ))}
...
const MapBadge = styled.div`
  position: relative; z-index: 2; background: #111; color: #fff; padding: 10px 20px; border-radius: 30px; font-weight: bold; font-size: 13px; display: flex; align-items: center; gap: 8px; border: 3px solid #fff; box-shadow: 0 4px 15px rgba(0,0,0,0.15);
`;
...
const WelcomeTag = styled.span`
  background: #A7D631; font-weight: bold; padding: 6px 14px; border-radius: 20px; font-size: 10px; display: inline-block; margin-bottom: 15px; color: #111;
`;
...
const ActionIcon = styled.div`
  width: 45px; height: 45px; border-radius: 50%; background: #A7D631; display: flex; justify-content: center; align-items: center; font-size: 16px; color: #111; margin-right: 15px;
`;
...
const ActionArrow = styled.div`
  width: 35px; height: 35px; border-radius: 50%; background: #111; color: #fff; display: flex; justify-content: center; align-items: center; font-size: 11px;
`;
...
const StatusCard = styled.div`
  flex: 1; background: #F8FBEF; border-radius: 20px; padding: 18px; display: flex; align-items: center; gap: 15px; border: 1px solid #f2f7e4;
  strong { display: block; font-size: 15px; margin-bottom: 4px; color: #111; }
  span { font-size: 12px; color: #7f7f7f; }
`;

const IconWrapper = styled.div`
  background: #fff; width: 40px; height: 40px; border-radius: 50%; display: flex; justify-content: center; align-items: center; font-size: 16px; color: #111; box-shadow: 0 2px 8px rgba(167,214,49,0.15);
`;
...
const SeeAll = styled.span`
  font-size: 13px; color: #7f7f7f; cursor: pointer; font-weight: 600; transition: 0.2s;
  &:hover { color: #A7D631; text-decoration: underline; }
`;

const ResponsiveList = styled.div<{ expandido: boolean }>`
  display: flex; 
  gap: 15px; 
...
const CardTag = styled.span<{ cor?: string }>`
  position: absolute; top: 25px; left: 25px; background: ${props => props.cor || '#A7D631'}; font-size: 10px; font-weight: bold; padding: 5px 10px; border-radius: 8px; color: #111; z-index: 2;
`;

const CardImage = styled.img`
  width: 100%; height: 130px; object-fit: cover; border-radius: 14px; margin-bottom: 12px; position: relative; z-index: 1;
`;
...
const CardButtonDark = styled.button`
  width: 100%; background: #111; color: #fff; padding: 12px; border: none; border-radius: 12px; font-weight: bold; cursor: pointer; transition: 0.2s; font-size: 12px;
  &:hover { background: #333; transform: translateY(-1px); }
`;

const CardButtonLight = styled.button`
  width: 100%; background: #edf5d3; color: #111; padding: 12px; border: none; border-radius: 12px; font-weight: bold; cursor: pointer; transition: 0.2s; font-size: 12px;
  &:hover { background: #e2f0b6; transform: translateY(-1px); }
`;